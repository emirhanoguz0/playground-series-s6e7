# Deney 10: pseudo-labeling
# exp3 modelinin cok emin oldugu test satirlari (max proba > 0.99)
# tahmin edilen etiketle egitime katilir. Dogrulama yarisma verisinde kalir.
...
# exp3'un test olasiliklari -> guvenli satirlar
tp3 = np.load(f"{WORK}/test_proba_features.npy")
conf_mask = tp3.max(axis=1) > CONF
pseudo_y = np.argmax(tp3, axis=1)[conf_mask]
X_pseudo = X_test[conf_mask]
print(f"pseudo satir sayisi: {conf_mask.sum()} / {len(X_test)} ({conf_mask.mean():.1%})", flush=True)
print(f"pseudo etiket dagilimi: {np.bincount(pseudo_y, minlength=3)}", flush=True)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
    X_tr = pd.concat([X.iloc[tr_idx], X_pseudo], axis=0, ignore_index=True)
...
sub.to_csv(f"{WORK}/submission_pseudo.csv", index=False)
np.save(f"{WORK}/oof_pseudo.npy", oof)
np.save(f"{WORK}/test_proba_pseudo.npy", test_proba)
