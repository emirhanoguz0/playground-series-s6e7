# Deney 7: orijinal veri ekleme
# College Student Health Behavior Dataset (50k, CC0) egitime katilir.
# Kural: orijinal SADECE egitim kismina eklenir; dogrulama (OOF)
# yarisma verisinde kalir -> exp3 ile adil karsilastirma.
# Ozellikler ve parametreler exp3 ile ayni, ayni CV bolmesi (seed=42).
import time
import numpy as np
import pandas as pd
...
orig = pd.read_csv(f"{DATA}/student_health_dataset_50k.csv")
orig = orig.drop(columns=["student_id", "timestamp"])
...
y_orig = orig["health_condition"].map(target_map).values
...
ftr = add_features(train)
fte = add_features(test)
forig = add_features(orig)
feat_cols = [c for c in ftr.columns if c not in ("id", "health_condition")]
X = ftr[feat_cols].copy()
X_test = fte[feat_cols].copy()
X_orig = forig[feat_cols].copy()

for c in feat_cols:
    if not is_numeric_dtype(X[c]):
        X[c] = X[c].astype("category")
        X_test[c] = pd.Categorical(X_test[c], categories=X[c].cat.categories)
        X_orig[c] = pd.Categorical(X_orig[c], categories=X[c].cat.categories)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
    X_tr = pd.concat([X.iloc[tr_idx], X_orig], axis=0, ignore_index=True)
    y_tr = np.concatenate([y[tr_idx], y_orig])
    model = XGBClassifier(**params)
    model.fit(X_tr, y_tr, eval_set=[(X.iloc[va_idx], y[va_idx])], verbose=False)
...
print(f"exp3 (orijinalsiz) idi:    0.94988")
...
sub.to_csv(f"{WORK}/submission_original.csv", index=False)
np.save(f"{WORK}/oof_original.npy", oof)
np.save(f"{WORK}/test_proba_original.npy", test_proba)
