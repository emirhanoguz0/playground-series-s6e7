# Deney 5: sadece cekirdek ozellikler
# Hipotez: 10 gurultu kolonu modeli yaniltir. Onlari at.
# Kalan: sleep_duration, stress_level, physical_activity_level
# + exp3'un turetilmis ozellikleri. Ayni CV bolmesi (seed=42).
import time
import numpy as np
import pandas as pd
...
print(f"ozellik sayisi: {len(feat_cols)} -> {feat_cols}", flush=True)
...
print(f"exp3 (tum ozellikler) idi: 0.94988")
...
sub.to_csv(f"{WORK}/submission_core.csv", index=False)
np.save(f"{WORK}/oof_core.npy", oof)
np.save(f"{WORK}/test_proba_core.npy", test_proba)
